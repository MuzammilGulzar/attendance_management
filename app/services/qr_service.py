...
#     7. Get teacher profile     — needed to set marked_by_id on record
#     8. Insert pending record   — status='pending', is_qr_scan=True
#                                  Does NOT call mark_attendance() because
#                                  that function rejects 'pending' status.
#                                  We write directly to DB with full audit fields.
...
#     # ── Step 7: Get teacher profile ──────────────────────────────────
#     # We need Teacher.id to set marked_by_id on the Attendance record.
#     # The session stores Teacher.id directly — simple lookup.
...
#     # ── Step 8: Insert pending record directly ───────────────────────
#     # WHY NOT call mark_attendance()?
#     #   mark_attendance() validates status against VALID_STATUSES which
#     #   explicitly excludes 'pending' (QR-only state). Calling it with
#     #   'pending' would return an error.
#     #
#     # WHY write directly to DB here?
#     #   We still get all the safety we need:
#     #   • Duplicate prevention: step 6 above checked for existing record.
#     #     The DB UniqueConstraint is also a final safety net.
#     #   • is_qr_scan=True flags this as a QR submission for the review page.
#     #   • status='pending' means it has zero effect on attendance %
#     #     until the teacher confirms it via the review flow.
#     #   • Low-attendance notifications are NOT triggered here —
#     #     they fire after the teacher reviews and confirms.
#     try:
#         record = Attendance(
#             student_id   = student.id,
#             subject_id   = subject.id,
...
#         db.session.rollback()
#         # UniqueConstraint violation means a concurrent scan beat us
#         # to it — treat as already-marked rather than an error.
#         existing_now = Attendance.query.filter_by(
#             student_id = student.id,
#             subject_id = subject.id,
#             date       = today,
#         ).first()
...
#                 'date'          : today,
#             }
#         return False, 'Could not save attendance. Please try again.'

#     # ── Step 9: Increment teacher's scan counter ──────────────────────
#     # Only increment on fresh successful insert.
#     increment_scan_count(session_id)

#     return True, {
#         'already_marked' : False,
#         'pending_review' : True,     # tells result template to show
#         'student_name'   : student.full_name,
#         'roll_number'    : student.roll_number,
#         'subject_name'   : subject.name,
#         'subject_code'   : subject.code,
...
UPDATED FLOW (Step B/C/4 redesign):
  Teacher generates QR  → session created here
  Student scans QR      → lands on public form (NO login required)
  Student types roll number + name
  mark_by_roll_number() verifies → inserts status='pending', is_qr_scan=True
  Student sees          → "Submitted! Awaiting teacher confirmation"
  Teacher sees badge    → "12 pending QR submissions — Review"
  Teacher reviews       → confirms each as present/leave/event/no_class
  attendance_service    → calculates % only after review is complete
"""
...
#------handle Replay session problem
def mark_by_logged_in_student(session_id: str, student) -> tuple:
    from app.models.subject import Subject
    from app.models.teacher import Teacher
    from app.models.attendance import Attendance
    from app import db
    ...
    if student.is_graduated:
        return False, 'Graduated students cannot mark attendance.'
    ...
    if not enrolled:
        return False, f'You are not enrolled in {subject.name} ({subject.code}).'
    ...
    teacher = Teacher.query.get(session['teacher_id'])
    if not teacher:
        return False, 'Teacher not found. Please ask your teacher to regenerate the QR.'
